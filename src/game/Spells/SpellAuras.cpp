        ...
        switch (spellProto->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DRUID:
            {
                ...
                if (spellProto->SpellFamilyFlags & uint64(0x000000000000800000))
                {
                    ...
                    m_modifier.m_amount += int32(caster->GetTotalAttackPowerValue(BASE_ATTACK) * cp * 0.02f);
                }
                break;
            }
            case SPELLFAMILY_ROGUE:
            {
                ...
                else if ((spellProto->SpellFamilyFlags & uint64(0x0010000)) && spellProto->Id == 27187)
                {
                    if (caster->GetTypeId() != TYPEID_PLAYER)
                        break;
                    m_modifier.m_amount += int32(caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.06f);
                }
                break;
            }
            default:
